...
class InvalidValueError(Exception):
    ...
    def __init__(self, value, param, object=None):
        ...
        if object is not None:
            message += f" {object.get_name()} of class {object.__class__.__name__} was passed {value} of type {type(value)}. {param} only accepts integers."
        else:
            message += f" {value} was passed of type {type(value)}. {param} only accepts integers"
...
class InvalidCard(Exception):
    ...
    """
    def __init__(self, card, target: str):
